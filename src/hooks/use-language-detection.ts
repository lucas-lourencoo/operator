...
function normalizeFlouriteLanguage(lang: string, code: string): SupportedLanguage {
  const lowerLang = lang.toLowerCase();
  const lowerCode = code.toLowerCase();
  
  // Manual overrides for common JS/TS patterns that might be misidentified
  // TypeScript specific syntax
  if (code.includes('interface ') || code.includes('type ') || (code.includes(':') && (code.includes('string') || code.includes('number') || code.includes('boolean')))) {
  ...
  if (lowerCode.includes('import {') || lowerCode.includes('export function') || lowerCode.includes('export const')) {
    if (lowerLang === 'kotlin' || lowerLang === 'java' || lowerLang === 'unknown') {
      return 'typescript';
    }
  }
  ...
  const match = SUPPORTED_LANGUAGES.find(l => 
    l.id.toLowerCase() === lowerLang || 
    l.name.toLowerCase() === lowerLang
  );
