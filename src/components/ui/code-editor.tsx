...
import { getShikiHighlighter, SUPPORTED_LANGUAGES, type SupportedLanguage } from '@/lib/shiki';
...
export function CodeEditor({ 
  ...
  return (
    <div className={cn("flex flex-col rounded-lg border border-border-primary bg-bg-input overflow-hidden shadow-2xl relative", className)}>
      {/* Window Header */}
          ...
          <div className="relative flex items-center">
            ...
            >
              {SUPPORTED_LANGUAGES.map((lang) => (
                <option key={lang.id} value={lang.id} className="bg-bg-card text-text-primary">
                  {lang.name}
                </option>
              ))}
            </select>
      ...
      <div className="flex max-h-[500px] overflow-y-auto relative custom-scrollbar">
        ...
        <div className="flex flex-col items-end gap-[2px] pt-6 pb-6 px-3 border-r border-border-primary bg-bg-page/30 text-right font-mono text-xs text-text-tertiary select-none min-w-[48px] sticky left-0 z-10">
          {Array.from({ length: lineCount }, (_, i) => i + 1).map((num) => (
            <span key={num} className="leading-5 h-5">{num}</span>
          ))}
        </div>
