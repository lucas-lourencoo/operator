    ...
    <>
      <div className="flex flex-col overflow-hidden rounded-lg border border-border-primary bg-bg-card">
        <div className="flex h-10 items-center border-b border-border-primary bg-bg-hover px-5">
          ...
          <span className="w-[100px] text-right font-mono text-xs font-medium text-text-tertiary">lang</span>
        </div>
        {[1, 2, 3].map((i) => (
          <TableRow key={i}>
...
export async function ShameLeaderboard() {
  ...
  const data = await queryClient.fetchQuery(api.roast.getLeaderboardPreview.queryOptions());
    ...
    <>
      <div className="flex flex-col overflow-hidden rounded-lg border border-border-primary bg-bg-card">
        ...
        {data.roasts.map((roast, index) => {
          ...
          return (
            <TableRow key={roast.id} className="items-start py-4 border-b border-border-primary last:border-b-0 px-5">
              <TableCell className={`w-[50px] font-medium ${rankColor} pt-[18px] items-start`}>
                {rank}
              </TableCell>
